...
if(empty($_POST["num"]) || empty($_POST["password"]))
{
    ...
    $sql = "SELECT student_id FROM users WHERE num='$username' and password='$password'";

    $result = mysqli_query($db, $sql);

    $q = mysqli_query($db, "SELECT uid, perm_id FROM users WHERE num = '$username' AND password = '$password' LIMIT 0,1");
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        $result1 = mysqli_query($db, "SELECT * from users WHERE num = '$username' and password = '$password'");
